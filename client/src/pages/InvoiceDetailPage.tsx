...
import { ArrowLeft, Send, DollarSign, Printer, Trash2, Edit } from 'lucide-react'
  ...
  return (
    <div className="p-8 max-w-4xl">
      ...
      <div className="flex items-center justify-between mb-6">
        <button onClick={() => navigate('/invoices')} className="flex items-center gap-1 text-sm text-slate-400 hover:text-slate-600 transition-colors">
        ...
        <div className="flex items-center gap-2">
          ...
          {invoice.status === 'draft' && (
            <Button variant="ghost" size="sm" icon={<Trash2 size={14} />} onClick={handleDelete} className="text-red-400 hover:text-red-600" />
          )}
        </div>
      </div>
      ...
      <Card className="print:shadow-none">
        <div className="p-8">
          ...
          <div className="flex justify-between items-start mb-10">
            <div>
              <h1 style={{ fontFamily: 'Playfair Display, serif' }} className="text-4xl font-bold text-[#0f172a]">
                INVOICE
              </h1>
              <p className="text-slate-400 mt-1 font-mono">{invoice.number}</p>
            </div>
          ...
          <div className="grid grid-cols-2 gap-8 mb-10">
            <div>
              ...
              <p className="font-semibold text-[#0f172a]">Your Business</p>
            ...
            {invoice.client && (
              <div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="font-semibold text-[#0f172a]">{invoice.client.name}</p>
            ...
            <thead>
              <tr className="border-b-2 border-[#0f172a]">
                {['Description', 'Qty', 'Unit Price', 'Tax', 'Total'].map((h) => (
                  <th key={h} className={`py-2 text-xs font-semibold text-slate-600 uppercase tracking-wide ${h === 'Description' ? 'text-left' : 'text-right'}`}>
                    {h}
                  </th>
                ))}
              </tr>
            ...
            <tbody className="divide-y divide-gray-50">
              {invoice.lineItems?.map((item, i) => (
                <tr key={i} className="group">
                  ...
                  <td className="py-3 text-sm text-slate-500 text-right">{formatCurrency(item.unitPrice, invoice.currency as CurrencyCode)}</td>
                  <td className="py-3 text-sm text-slate-500 text-right">
                    {item.taxRate ? `${(item.taxRate * 100).toFixed(0)}%` : '—'}
                  </td>
                  <td className="py-3 text-sm font-medium text-[#0f172a] text-right">
          ...
          <div className="flex justify-end">
            <div className="w-64 space-y-2">
              ...
              <div className="flex justify-between font-bold text-[#0f172a] border-t-2 border-[#0f172a] pt-2">
                <span>Total</span>
