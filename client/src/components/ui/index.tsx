...
export const Button = forwardRef<HTMLButtonElement, ButtonProps>(
  ({ variant = 'primary', size = 'md', loading, icon, children, className, disabled, ...props }, ref) => {
    ...
    const variants = {
      primary: 'bg-[#0f172a] text-white hover:bg-[#1e2a45] focus:ring-[#0f172a]',
      secondary: 'bg-white text-[#0f172a] border border-gray-200 hover:bg-gray-50 focus:ring-gray-300',
    ...
    return (
      <div className="flex flex-col gap-1">
        ...
        <input
          ...
          id={inputId}
          className={cn(
            'w-full px-3 py-2 text-sm bg-white border rounded-lg text-[#0f172a] placeholder:text-slate-400',
            'focus:outline-none focus:ring-2 focus:ring-[#0f172a]/20 focus:border-[#0f172a]',
            'transition-colors',
    ...
    return (
      <div className="flex flex-col gap-1">
        ...
        <select
          ...
          id={inputId}
          className={cn(
            'w-full px-3 py-2 text-sm bg-white border rounded-lg text-[#0f172a]',
            'focus:outline-none focus:ring-2 focus:ring-[#0f172a]/20 focus:border-[#0f172a]',
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className={cn('relative bg-white rounded-2xl shadow-2xl w-full', width)}>
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <h2 style={{ fontFamily: 'Playfair Display, serif' }} className="text-lg font-semibold text-[#0f172a]">
            {title}
          </h2>
...
export function StatCard({ label, value, sub, accent }: {
  ...
  return (
    <Card className={cn('px-5 py-5', accent && 'bg-[#0f172a] border-0')}>
      <p className={cn('text-xs font-medium uppercase tracking-widest', accent ? 'text-slate-400' : 'text-slate-500')}>
        {label}
      </p>
      <p className={cn('text-2xl font-bold mt-1', accent ? 'text-white' : 'text-[#0f172a]')}
        style={{ fontFamily: 'Playfair Display, serif' }}>
        {value}
      </p>
      {sub && <p className={cn('text-xs mt-1', accent ? 'text-slate-400' : 'text-slate-400')}>{sub}</p>}
    </Card>
  )
}
